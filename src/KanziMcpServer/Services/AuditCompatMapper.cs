...
/// Keep in sync with KanziMcpPlugin.Services.AuditCompatMapper.
/// </summary>
public static class AuditCompatMapper
{
    ...
    public static object BuildDoctorArgs(bool checkUnused, bool checkBroken, bool checkOrphaned)
    {
        _ = checkOrphaned;
        return new
        {
    ...
    public static string MapDoctorJsonToResourceReferencesCompat(string doctorJson, bool checkOrphaned)
    {
        ...
        if (checkOrphaned)
        {
            foreach (var res in unusedResources)
            {
                ...
                {
                    ["type"] = res.GetValueOrDefault("type"),
                    ["path"] = res.GetValueOrDefault("path"),
                    ["name"] = res.GetValueOrDefault("name"),
                    ["message"] = $"{res.GetValueOrDefault("type")} '{res.GetValueOrDefault("name")}' is not referenced by any node"
                });
            }
        }

        var detectedTypes = unusedResources
            .Select(r => r.GetValueOrDefault("type")?.ToString() ?? "")
